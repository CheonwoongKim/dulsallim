...
/**
 * 다시 그린 뒤 불러 준다. 여기서는 붙음만 다시 본다.
 *
 * 접거나 펴는 판단은 하지 않는다. 다시 그리는 순간의 scrollY 는 사용자가 고른 자리가
 * 아닐 수 있어서다. 캘린더로 바꾸면 목록이 숨어 문서가 짧아지고, 스크롤할 데가 없어지면
 * 브라우저가 위치를 0 으로 되감는다. 그 상태에서 날짜를 고르면 목록이 다시 나오며 문서가
 * 길어지는데, 그때 scrollY 0 을 "맨 위로 올렸다"로 읽어 머리가 저절로 펴졌다
 * (접힘 → scrollY 252 → 캘린더에서 0 으로 되감김 → 날짜 선택으로 문서만 길어짐).
 *
