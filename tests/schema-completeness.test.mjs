...
/**
 * 표마다, 마이그레이션 전체가 그 표에 거는 check 식을 모은다.
 *
 * 칼럼 정의에 붙여 적기도 하고(`add column ... check (...)`) 따로 떼어 적기도 한다
 * (`add constraint ... check (...)`). 거는 자리가 어디든 운영 DB 에 서는 것은 같으므로
 * 한 자루에 담아 두고, 스키마 쪽 제약이 이 자루 안에 있는지만 본다.
 */
function 마이그제약모음(마이그레이션들) {
  const 자루 = {};
  for (const [, 글] of 마이그레이션들) {
    for (const [, 표, 몸] of 알맹이(글).matchAll(/alter\s+table\s+([a-z_][a-z0-9_]*)([^;]*);/gi)) {
      자루[표] ??= new Set();
      for (const 검사 of 검사조각들(몸)) 자루[표].add(검사);
    }
  }
  return 자루;
}

const 마이그제약 = 마이그제약모음(마이그레이션들);
...
test("마이그레이션이 세우는 칼럼은 schema.sql 의 것과 같다", () => {
  ...
  for (const [파일, 글] of 마이그레이션들) {
    for (const [, 표, 정의원본] of 알맹이(글).matchAll(더하는칼럼)) {
       ...
       * 양쪽으로 다 본다.
       *
       * 마이그레이션이 **더** 말하면 운영 DB 에만 있는 제약이 생기고, **덜** 말하면
       * 운영 DB 에만 없는 제약이 생긴다. 둘 다 새로 깐 DB 와 어긋나는 것은 같은데,
       * 한동안 "마이그가 적은 것만" 훑어서 덜 말하는 쪽이 통째로 빠져 있었다 —
       * 마이그에서 check 를 지우면 돌 것이 없어 그냥 통과했다.
       */
      const 양쪽 = (있나, 무엇) => {
        if (있나(마이그) && !있나(스키마것)) 어긋난것.push(`${파일}: ${표}.${이름} — ${무엇} 이 마이그에만 있다`);
        if (!있나(마이그) && 있나(스키마것)) 어긋난것.push(`${파일}: ${표}.${이름} — ${무엇} 이 schema.sql 에만 있다`);
      };
      양쪽((글) => 글.includes(" not null"), "not null");

      const 기본값 = (글) => /\bdefault\s+('[^']*'|[^\s,]+)/.exec(글)?.[0] ?? "";
      if (기본값(마이그) !== 기본값(스키마것)) {
        어긋난것.push(`${파일}: ${표}.${이름} 의 기본값이 다르다 — 마이그 "${기본값(마이그)}" · schema "${기본값(스키마것)}"`);
      }

      // 마이그가 거는 check 는 schema 에도 있어야 한다.
      for (const 검사 of 검사조각들(정의)) {
        if (!스키마것.includes(검사)) 어긋난것.push(`${파일}: ${표}.${이름} 의 ${검사} 가 schema.sql 에 없다`);
      }
      // 그리고 schema 가 거는 check 는 마이그레이션 어딘가가 걸고 있어야 한다.
      for (const 검사 of 검사조각들(스키마정의)) {
        if (!마이그제약[표]?.has(검사)) {
          어긋난것.push(`${표}.${이름} 의 ${검사} 를 거는 마이그레이션이 없다 — 운영 DB 에는 이 제약이 없다`);
        }
      }
    }
  }
  ...
  assert.ok(마이그제약.fixed_costs?.size > 0, "마이그레이션의 제약을 한 건도 못 모았다");
});
