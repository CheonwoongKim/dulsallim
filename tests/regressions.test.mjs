...
test("고정비는 한 트랜잭션으로 반영해 같은 달이 두 번 들어가지 않는다", async () => {
  ...
  // 잘린 건수도 함께 실어 보낸다. 무엇을 채우는 중인지(due) 알아야 이번에 물었는지 안다.
  assert.match(반영, /countSkippedMonths\(templates, applied, due\)/);
...
test("제목 줄에는 이제 제목과 보기 방식뿐이다", () => {
   ...
   * 지출 목록이 그만큼 올라와 393 에서 5줄이라는 약속이 깨진다. 제목 단추가 그 높이를 물려받는다.
   */
...
test("거르기 시트는 분류만 맡는다", () => {
  ...
  /*
   * 그 달에 쓴 분류만, 많이 쓴 순 — 분석 화면과 같은 함수로 낸다.
   * 여기서 따로 세던 때는 합계가 두 곳에서 나와, 무엇을 더할지가 바뀌면
   * (실비 환급이 그랬다) 한쪽만 고쳐질 자리였다. 차례와 금액은 analysis.test.mjs 가 본다.
   */
  assert.match(app, /const 이번달분류 = \(\) => sumByCategory\(/);
  /*
...
test("총액 글자는 화면 폭 따라 흔들리지 않는다", () => {
  /*
   * clamp(41px, 12.6vw, 58px) 이던 때는 같은 금액이 주연 폰(393)에서 49.5, 천웅 폰(430)
   * 에서 54.2 로 달라 보였다. 늘어난 것은 글자 길이가 아니라 화면 폭이라, 자리가 모자라서
   * 줄인 적은 한 번도 없다 — 393 에서 쓸 수 있는 폭 345 에 견주면 늘 한참 남았다.
   */
  const 값 = css.match(/--text-hero: ([^;]+);/)[1].trim();
  assert.doesNotMatch(값, /clamp|vw|vh|vmin|%/, "총액이 다시 화면 폭을 따라간다");

  const px = Number(값.match(/^(\d+)px$/)?.[1]);
  assert.ok(px, `--text-hero 가 고정 px 이 아니다: ${값}`);

  /*
   * 그래도 넘치지는 않아야 한다. 있을 법한 가장 큰 금액(99,999,999원)이 48px 에서 236px
   * 이었다(WebKit 계측). 폭은 글자 크기에 비례하므로 그 비율로 되짚는다.
   */
  const 가장긴것 = 236 / 48;
  const 좁은쪽 = 345; // 393 폰에서 hero 가 쓸 수 있는 폭
  assert.ok(px * 가장긴것 <= 좁은쪽,
    `${px}px 이면 가장 큰 금액이 ${Math.ceil(px * 가장긴것)}px — 좁은 폰의 ${좁은쪽} 을 넘는다`);

  // 계단은 34 에서 끝난다. 총액은 그보다 커야 총액이다.
  const 계단끝 = Number(css.match(/--text-34: (\d+)px/)[1]);
  assert.ok(px > 계단끝, `총액이 가장 큰 제목(${계단끝})보다 크지 않다`);
});
...
test("탭 줄은 머리 아래에 붙어 있어 어디까지 내려가도 누를 수 있다", () => {
  /*
   ...
   */
  assert.match(css, /\.segmented-control\.segment-tabs \{[^}]*position: sticky/, "탭 줄은 붙어 있어야 한다");
  assert.match(css, /\.segmented-control\.segment-tabs \{[^}]*top: var\(--head-height\)/, "머리 바로 아래에 붙는다");
  /*
   ...
   */
  assert.doesNotMatch(css, /\n\.segment-tabs \{/, "이름 하나로 걸면 .segmented-control 이 이긴다");
...
test("본 화면 단락 사이 여백은 머리를 바꾸기 전과 같다", () => {
   ...
   *
   * hero 아래는 32 에서 20 으로 내렸다. 32 였을 때는 요약의 위 20 과 합쳐 52 라 금액
   * 글자에서 사람 이름까지 55px 이 비어, 이 화면에서 가장 큰 틈이었다(계측). 지금은 43px.
   */
  assert.match(css, /\.month-bar \{[^}]*padding: var\(--space-2\) var\(--space-6\) var\(--space-2\)/);
  assert.match(css, /\n\.hero \{[^}]*padding: var\(--space-6\) 0 var\(--space-5\)/, "휴대폰: 위로 24px, 아래로 20px");
...
test("여럿 중 하나 고르는 부품은 같은 모서리 계단을 쓴다", () => {
   ...
   * 되고, 제목 줄이 62 에서 70 으로 늘어 지출 목록이 8px 밀린다 — 393 에서 5줄이라는
...
test("쓰지 않게 된 모양새와 손잡이를 남기지 않는다", () => {
   ...
   */
  /*
   * 아이디를 먼저 지운다. 이름이 같으면(.wish-note 와 #wish-note) 죽은 규칙이 살아 있는
   * 것으로 잘못 읽힌다 — 실제로 그렇게 한동안 남아 있었다.
   *
   * 화면의 id="..." 와 코드의 "#..." 둘 다 지워야 한다. 한쪽만 지우면 다른 쪽이 살려 놓는다.
   */
  const 부르는곳 = `${app}\n${html}`
    .replace(/#[a-z][\w-]*/g, " ")
    .replace(/\bid="[^"]*"/g, " ");
...
test("뿌리에 무엇이 왜 있는지 다 설명된다", async () => {
  ...
  const 있어야할것 = {
    ...
    tests: "검사 — node 로 도는 것",
    "tests-browser": "검사 — 진짜 브라우저로 도는 것 (npm run check)",
    tools: "목 서버와 굽는 도우미 — 앱에는 안 실린다",
    "README.md": "처음 까는 법",
