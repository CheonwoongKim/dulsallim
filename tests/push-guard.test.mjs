...
test("길이가 다른 것을 같다고 하지 않는다", () => {
  /*
   * 길이를 씨앗으로 안 넣으면 짧은 쪽의 없는 자리가 0 으로 읽혀, 열쇠 뒤에 널 문자를
   * 붙인 토큰이 통과한다. 그 줄을 "간단히" 지워도 아무 검사가 안 울던 자리다.
   */
  assert.equal(서버인가(`Bearer ${서버열쇠}\u0000`, 서버열쇠), false, "뒤에 널 문자를 붙였는데 통과했다");
  assert.equal(서버인가(`Bearer ${서버열쇠}\u0000\u0000`, 서버열쇠), false);
