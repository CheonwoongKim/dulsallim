...
<details>
<summary>운영 중인 프로젝트를 최신으로 올릴 때 — 마이그레이션 21개</summary>
...
| 21 | `supabase/migrations/20260922010000_expense_refund.sql` | 실비 환급 — 결제 금액은 두고 돌려받은 금액을 따로 적는다 |
...
5. 이어서 `20260922010000_expense_refund.sql` 도 실행합니다.

   > 위에서 "새 프로젝트에는 마이그레이션을 실행하지 마세요" 라고 했는데 이 셋(8·9·21)만 예외입니다.
   > 알림 발송에 필요한 것은 비밀값 둘을 손으로 채워야 해서 `schema.sql` 에 못 넣어 뒀고,
   > 그래서 9번을 직접 실행하게 됩니다. 그런데 9번의 월말 요약은 결제 금액을 그대로 더하는
   > 옛 판이라, 21번이 그 함수를 실부담(`amount - refunded`)으로 덮습니다.
   > 9번만 실행하면 화면은 3만 원이라 하는데 알림만 10만 원을 말합니다.
