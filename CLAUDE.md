...
**칼럼도 같은 문을 지난다.** 한동안 함수만 견주고 칼럼은 이름조차 안 봐서, `add column`
줄을 통째로 지워도 `smallint` 를 `integer` 로 바꿔도 검사 전체가 통과했다. 지금은
`tests/schema-completeness.test.mjs` 가 마이그레이션이 세우는 칼럼의 **형·not null·기본값·
check 식**을 `schema.sql` 의 같은 칼럼과 **양쪽으로** 맞춰 보고, 주석만 남아 실행할 것이 없는
마이그레이션도 잡는다.

양쪽이라는 말이 중요하다. 마이그레이션이 schema 보다 **더** 말하면 운영 DB 에만 있는 제약이
생기고, **덜** 말하면 운영 DB 에만 없는 제약이 생긴다. 한동안 "마이그가 적은 것만" 훑어서
덜 말하는 쪽이 통째로 빠져 있었다 — 마이그레이션에서 `check` 를 지우면 견줄 것이 없어
그냥 통과했다. 따로 떼어 건 `add constraint` 도 함께 센다.
